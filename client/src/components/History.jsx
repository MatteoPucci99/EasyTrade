                    ...
                    <Col className="mt-3" style={{ overflowX: 'auto' }} >
                      <div style={{width:'1200px'}}>
                        <Row className="border-bottom border-secondary">
                            ...
                            <Col><p className="fw-bold">Data</p></Col>
                            <Col><p className="fw-bold">Rischio Percentuale</p></Col>
                            <Col><p className="fw-bold">Risultato Finale</p></Col>
                            <Col><p className="fw-bold">Esito</p></Col>
                            <Col></Col>
                        </Row>
                        {trades.length>0 ? (trades.slice(currentIndex, currentIndex + selectedIndex).map((trade) => (<Row className="border-bottom border-secondary hover" style={{cursor:'pointer'}}>
                            <Col className="mt-3"><p className="fw-semibold">{trade.pair.toUpperCase()}</p></Col>
                            <Col className="mt-3"><p className={`fw-semibold ${trade.type === 'Vendi' ? 'sellType' : 'buyType'}`}>{trade.type}</p></Col>
                            <Col className="mt-3"><p className="fw-semibold">{trade.date}</p></Col>
                            <Col className="mt-3"><p className="fw-semibold">{trade.risk}%</p></Col>
                            <Col className="mt-3"><p className="fw-semibold">{trade.reward}%</p></Col>
                            <Col className="mt-3"><p className={`fw-semibold ${trade.result === 'Stop Loss' ? 'sl' : trade.result === 'Take Profit' ? 'tp' : 'be'}`}>{trade.result}</p></Col>
                            <Col className="mt-3 d-flex">
                                <div style={{ borderRadius: '50%', height: '30px', width: '30px' }} className="hoverIcon d-flex justify-content-center align-items-center me-2">
                                    <RiPencilFill style={{fontSize:'1.15em', cursor:'pointer'}}  onClick={()=>{handleShowModal(trade)}} />
                                </div>
                                <div style={{ borderRadius: '50%', height: '30px', width: '30px' }} className="d-flex justify-content-center align-items-center hoverIcon">
                                   <RiDeleteBin5Line style={{fontSize:'1.15em', color:'rgb(179,58,60)', cursor:'pointer'}}  onClick={()=>{showDeleteModal(trade)}}/>
                                </div>
                            </Col>
                        </Row>))) : (null)}
                      </div>
                      ...
                      <Row className="d-flex justify-content-end mt-3">
                            <Col xs={12} sm={5} md={4} lg={3} className="d-flex justify-content-center align-items-center">
                                ...
                                <Dropdown>
                                  ...
                                  <Dropdown.Menu>
                                    <Dropdown.Item  onClick={()=>setSelectedIndex(5)} >5</Dropdown.Item>
                                    <Dropdown.Item onClick={()=>setSelectedIndex(10)}>10</Dropdown.Item>
                                    <Dropdown.Item onClick={()=>setSelectedIndex(15)}>15</Dropdown.Item>                                    
                                  </Dropdown.Menu>
                                </Dropdown>
                            </Col>
                            <Col xs={12} sm={5} md={4} lg={2} className="d-flex align-items-center justify-content-center">
                                <div style={{ borderRadius: '50%', height: '30px', width: '30px' }} className="hover d-flex justify-content-center align-items-center">
                                    ...
                                    <p className="mb-0 mx-2">{startTradeIndex}-{endTradeIndex} di {trades.length}</p>
                                <div style={{ borderRadius: '50%', height: '30px', width: '30px' }} className="hover d-flex justify-content-center align-items-center">
            ...
            <Modal show={showDelete} onHide={hideDeleteModal}>
                <Container className="darkthemeBgCards darkthemeText rounded-1 p-3">
                  ...
                  <div className="d-flex justify-content-end">
                    <Button variant="outline-danger" onClick={()=>{hideDeleteModal(); deleteTrade()}}>Conferma</Button>
                  </div>                 
                </Container>
                ...
                <form onSubmit={handleFormSubmit}>
                    <div className="d-flex justify-content-center">
                        <div id="tradeButtonContainer">
                            <button className={`tradeButton buy ${selectedButton === 'Compra' ? 'selectedBuy' : ''}`} onClick={(e)=>{e.preventDefault();setSendTrade({...sendTrade, type:'Compra'}); setSelectedButton('Compra')}}>Compra</button>
                            <button className={`tradeButton sell ${selectedButton === 'Vendi' ? 'selectedSell' : ''}`} onClick={(e)=>{e.preventDefault();setSendTrade({...sendTrade, type:'Vendi'}); setSelectedButton('Vendi') }}>Vendi</button>
                        </div>
                    </div>
                     ...
                     <Dropdown className="d-flex justify-content-center mt-3">
                      ...
                      <Dropdown.Menu>
                        {props.singleStrategy.params.pair.filter(pair=> pair !== pairSelected).map((el,index)=>{
                         return(
                         <Dropdown.Item key={index} onClick={(e)=>{setPairSelected(el); setSendTrade({...sendTrade, pair: e.target.innerText}) }} >{el}</Dropdown.Item>
                         )
                        })}
                      </Dropdown.Menu>
                     </Dropdown>
                     ...
                     <Dropdown className="d-flex justify-content-center mt-3">
                      ...
                      <Dropdown.Menu>
                        {tradeType.filter(type=> type !== typeStatus).map((el,index)=>{
                         return(
                         <Dropdown.Item key={index} onClick={(e)=>{setTypeStatus(el); setSendTrade({...sendTrade, result: e.target.innerText})}} >{el}</Dropdown.Item>
                         )
                        })}
                      </Dropdown.Menu>
                     </Dropdown>
                     ...
                     <div className="d-flex justify-content-center">
                        ...
                        <div className="input-container mt-3 ">
                           <input type="text" placeholder=" " value={sendTrade ? sendTrade.reward : ''} onChange={handleInputChange}/>
                           <label>Esito %</label>
                     ...
                     <div className="d-flex justify-content-center ">
                        <div style={{width:'80%'}} className="input-container mt-3">
                           <div >
                              {<input type="date" placeholder=" " value={sendTrade.date ? sendTrade.date : ''}/>}                     
                              <label>Data</label>
